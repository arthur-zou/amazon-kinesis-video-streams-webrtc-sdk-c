...
typedef struct {
    ...
    PStunPacket bindingResponseList[DEFAULT_NAT_TEST_MAX_BINDING_REQUEST_COUNT * 6];
        ...
        PKvsIpAddress pSrc, PKvsIpAddress pDest)
{
    ...
    if (pNatTestData->bindingResponseCount < DEFAULT_NAT_TEST_MAX_BINDING_REQUEST_COUNT * 3) {
                      ...
                      UINT32 testIndex, PNatTestData pData, PStunPacket *ppTestReponse)
{
    ...
    for (i = 0; testResponse == NULL && i < DEFAULT_NAT_TEST_MAX_BINDING_REQUEST_COUNT; ++i) {
        ...
        CVAR_WAIT(pData->cvar, pData->lock, 500 * HUNDREDS_OF_NANOS_IN_A_MILLISECOND);
        if (pData->bindingResponseCount > 0) {
            for (j = 0; j < pData->bindingResponseCount; ++j) {
                if (getInt32(*(PINT32) pData->bindingResponseList[j]->header.transactionId) == testIndex) {
...
STATUS natMappingBehaviorTest(PIceServer pStunServer, PNatTestData data, PSocketConnection pSocketConnection,
...
STATUS natFilteringBehaviorTest(PIceServer pStunServer, PNatTestData data, PSocketConnection pSocketConnection,
                           ...
                           UINT64 filterFuncCustomData)
{
    ...
    CHK_STATUS(natMappingBehaviorTest(&iceServerStun, &customData, pSocketConnection, pNatMappingBehavior));
    ...
    CHK_STATUS(natFilteringBehaviorTest(&iceServerStun, &customData, pSocketConnection, pNatFilteringBehavior));
